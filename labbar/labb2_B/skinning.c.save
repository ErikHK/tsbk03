...
void DeformCylinder()
{
	...
	for (row = 0; row < kMaxRow; row++)
	{
		for (corner = 0; corner < kMaxCorners; corner++)
		{
			...
			g_vertstmp0[row][corner] = MultVec3(Mult(T0b, 
			Mult(g_bones[0].rot, invT0b)), 
			g_vertsOrg[row][corner]);
			...
			//g_vertsRes[row][corner].z =(g_vertstmp1[row][corner].z) *(weight[row]);
			...
			//g_vertsRes[row][corner].z += g_vertstmp0[row][corner].z *(1-weight[row]);
...
void setBoneLocation(void)
{
	...
	// till vertexshadern
	mat4 tmp = T(g_bones[0].pos.x, g_bones[0].pos.y, g_bones[0].pos.z);
	glUniformMatrix4fv(glGetUniformLocation(g_shader, "g_bones_0_pos"), 1, GL_TRUE, tmp.m);
	tmp = T(g_bones[1].pos.x, g_bones[1].pos.y, g_bones[1].pos.z);
	glUniformMatrix4fv(glGetUniformLocation(g_shader, "g_bones_1_pos"), 1, GL_TRUE, tmp.m);
}
