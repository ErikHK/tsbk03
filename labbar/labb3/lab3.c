...
void updateWorld()
{
	...
	for (i = 0; i < kNumBalls; i++)
		...
		//if(i != j)
		//{
			vec3 b1 = {ball[i].X.x, 0, ball[i].X.z};
			vec3 b2 = {ball[j].X.x, 0, ball[j].X.z};

			if( Norm(VectorSub(b1, b2)) < kBallSize*2 )
			{
				vec3 p = {0, 0, 0};
				
				p.x = (ball[i].X.x+ball[j].X.x)/2.0f;
				...
				float jj = vrel* (-(eps+1))/(1/ball[i].mass + 1/ball[j].mass + DotProduct(nA,crossA) + DotProduct(nA, crossB) );

				//printf("%f, %f, %f;%f, %f, %f\n", rA.x, rA.y, rA.z, rB.x, rB.y, rB.z);

				//float jj = vrel* (-(eps+1))/(1/ball[i].mass + 1/ball[j].mass + 1);

				ball[i].P = VectorAdd(ball[i].P, ScalarMult(nA, jj));
				ball[j].P = VectorAdd(ball[j].P, ScalarMult(nA, -jj));
